...
//  SuccessQuotes
//
//  Created by Barbara on 08/12/2024.
//
...
struct InstagramShareView: View {
    ...
    var imageToShare: UIImage {
         // Convert your asset image to UIImage
         return UIImage(named: "steve")!
     }
    
  var body: some View {
      VStack {
                  // Convert UIImage back to SwiftUI Image for display
                  Image(uiImage: imageToShare)

                  if InstagramSharingUtils.canOpenInstagramStories {
                      Button(action: { InstagramSharingUtils.shareToInstagramStories(imageToShare) }) {
